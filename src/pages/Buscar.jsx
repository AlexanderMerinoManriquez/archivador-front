...
export default function Buscar() {
  ...
  const [modalNuevo, setModalNuevo] = useState(() => params.get('nueva') === '1' && permisos.digitalizar)
  const busqueda = useBusquedaPublica()

  const enviar = async (e) => {
    const expediente = await busqueda.buscar(e)
    if (expediente) navigate(RUTAS.expediente(expediente.rol))
  }

  const rolEscrito = busqueda.rol.trim()

  return (
    <main className="flex min-h-screen flex-col items-center justify-center px-4 pb-[8vh]">
      <div className="w-full max-w-3xl">
        <h1 className="mb-9 text-center text-3xl font-semibold tracking-tight text-slate-900 lg:text-4xl">Buscar Causa</h1>
        ...
        {permisos.digitalizar && (
          <div className="mt-9 flex flex-wrap items-center justify-center gap-3">
            <MenuRecientes onAbrir={(exp) => navigate(RUTAS.expediente(exp.rol))} />
